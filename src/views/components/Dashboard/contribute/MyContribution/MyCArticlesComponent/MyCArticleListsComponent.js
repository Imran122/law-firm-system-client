...
import MyCArticlesModalComponent from "./MyCArticlesModalComponent";

const MyCArticleListsComponent = () => {
  const [articleListData, setArticleListData] = useState([]);
  useEffect(() => {
    fetch(`${process.env.REACT_APP_API}/api/get-contribute-article-by-user`, {
      method: "GET",
      headers: {
      ...
      .then((data) => setArticleListData(data.result));
  }, []);
      ...
      <div className="row">
        <div className="col col-12">
          <div className="users-tbl-box">
            {articleListData.map((data) => (
              <div className="row books-tbl mt-3 br-bottom" key={data._id}>
                <div className="col col-6 col-lg-8">
                  <Link to={"/"}>
                    <h3>{data.title}</h3>
                  </Link>
                  <p>{data.journalOverview}</p>
                  <h5>
                    {data.authorName} · {data.journalArticleCategorey} ·{" "}
...
export default MyCArticleListsComponent;
