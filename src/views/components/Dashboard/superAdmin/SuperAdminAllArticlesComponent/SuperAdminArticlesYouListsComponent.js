...
import AllArticlesByYouModalComponent from "./AllArticlesByYouModalComponent";
import MyCArticlesModalComponent from "../../contribute/MyContribution/MyCArticlesComponent/MyCArticlesModalComponent";
const SuperAdminArticlesYouListsComponent = () => {
  const [articleList, setArticleList] = useState([]);
  const { isLoading, setIsLoading } = useAuth();
  useEffect(() => {
    fetch(
      `${process.env.REACT_APP_API}/api/get-contribute-journal-article-byadmin`,
      {
        method: "GET",
        headers: {
  ...
  console.log("allclcik", clickedArticleId);
  ...
  return (
    <>
          ...
          <div className="users-tbl-box">
            {articleList.map((data) => (
              <div className="row arti-tbl mt-3" key={data._id}>
                <div className="col col-11 col-lg-8">
                  <div className="form-check">
                    <input
                      className="form-check-input"
                      type="checkbox"
      ...
      <Modal
        show={articles}
        onHide={closeArticles}
        aria-labelledby="contained-modal-title-vcenter"
        centered
      >
        <MyCArticlesModalComponent
...
export default SuperAdminArticlesYouListsComponent;
