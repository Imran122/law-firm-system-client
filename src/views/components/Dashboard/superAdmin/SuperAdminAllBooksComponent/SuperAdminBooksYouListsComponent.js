...
import AllBooksByYouModalComponent from "./AllBooksByYouModalComponent";
import MyCBooksModalComponent from "../../contribute/MyContribution/MyCBooksComponent/MyCBooksModalComponent";
const SuperAdminBooksYouListsComponent = () => {
  const [lawsList, setLawsList] = useState([]);
  const { isLoading, setIsLoading } = useAuth();
  useEffect(() => {
    fetch(`${process.env.REACT_APP_API}/api/get-contribute-book-byadmin`, {
      method: "GET",
      headers: {
  ...
  const updatePaidOrFreeSatus = (id, status) => {
    ...
    fetch(`${process.env.REACT_APP_API}/api/update-status-paid-free`, {
      ...
      .then((data) => {
        if (data) {
          setIsLoading(false);
          navigate("/dashboard/all-books", { replace: true });
        }
      });
  };
  ...
  return (
    <>
          ...
          <div className="users-tbl-box">
            {lawsList.map((data) => (
              <div className="row books-tbl mt-3" key={data._id}>
                ...
                <button className="btn btn-info" onClick={() => editBooks(data)}>Edit</button>
                </div>
                <div className="col col-4 col-lg-1">
                  <button
      ...
      <Modal
          ...
          centered
        >
            <MyCBooksModalComponent setBooks={setBooks} contributeEdit={contributeEdit}/>
        </Modal>
    </>
  );
};

export default SuperAdminBooksYouListsComponent;
