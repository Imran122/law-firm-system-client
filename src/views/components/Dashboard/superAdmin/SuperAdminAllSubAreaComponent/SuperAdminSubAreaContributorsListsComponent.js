...
const SuperAdminSubAreaContributorsListsComponent = () => {
  const [lawsList, setLawsList] = useState([]);
  const { isLoading, setIsLoading } = useAuth();
  useEffect(() => {
    fetch(`${process.env.REACT_APP_API}/api/get-subject-area-by-contributor`, {
      method: "GET",
      headers: {
  ...
  const updatePaidOrFreeSatus = (id, status) => {
    ...
    fetch(`${process.env.REACT_APP_API}/api/update-sub-area-single-status`, {
      ...
      .then((data) => {
        if (data) {
          setIsLoading(false);
          navigate("/dashboard/all-subarea/by-contributors", { replace: true });
        }
      });
  };
  ...
  const sendSingleDataToTrash = (id, status) => {
    ...
    fetch(`${process.env.REACT_APP_API}/api/send-single-subject-datato-trash`, {
      ...
      .then((data) => {
        if (data) {
          setIsLoading(false);
          navigate("/dashboard/all-subarea/by-contributors", {
            replace: true,
          });
        }
      });
...
export default SuperAdminSubAreaContributorsListsComponent;
