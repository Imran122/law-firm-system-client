...
const SuperAdminCaseLawContributorsListsComponent = () => {
  const [contributeList, setContributeList] = useState([]);
  const { isLoading, setIsLoading } = useAuth();
  useEffect(() => {
    fetch(`${process.env.REACT_APP_API}/api/get-case-law-by-contributor`, {
      method: "GET",
      headers: {
  ...
  const updatePaidOrFreeSatus = (id, status) => {
    ...
    fetch(`${process.env.REACT_APP_API}/api/update-status-paid-free`, {
      ...
      .then((data) => {
        if (data) {
          setIsLoading(false);
          navigate("/dashboard/all-case-law/by-contributors", {
            replace: true,
          });
        }
      });
...
export default SuperAdminCaseLawContributorsListsComponent;
