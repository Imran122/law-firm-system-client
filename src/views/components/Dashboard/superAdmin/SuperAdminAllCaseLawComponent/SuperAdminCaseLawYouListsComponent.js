...
import AllCaseLawByYouModalComponent from "./AllCaseLawByYouModalComponent";
import MyCCasesModalComponent from "../../contribute/MyContribution/MyCCasesComponent/MyCCasesModalComponent";
const SuperAdminCaseLawYouListsComponent = () => {
  const [contributeList, setContributeList] = useState([]);
  const { isLoading, setIsLoading } = useAuth();
  useEffect(() => {
    fetch(`${process.env.REACT_APP_API}/api/get-contribute-case-law-byadmin`, {
      method: "GET",
      headers: {
  ...
  const updatePaidOrFreeSatus = (id, status) => {
    ...
    fetch(`${process.env.REACT_APP_API}/api/update-status-paid-free`, {
      ...
      .then((data) => {
        if (data) {
          setIsLoading(false);
          navigate("/dashboard/all-case-law", { replace: true });
        }
      });
  };

  const [caseLaw, setCaseLaw] = useState(false);

  const closeCaseLaw = () => setCaseLaw(false);
  const showCaseLaw = () => setCaseLaw(true);
  ...
  const sendMultipleDataTrashSatus = (status) => {
    ...
    fetch(`${process.env.REACT_APP_API}/api/send-multiple-datato-trash`, {
      ...
      .then((data) => {
        if (data) {
          setIsLoading(false);
          navigate("/dashboard/all-case-law", {
            replace: true,
          });
        }
      });
                ...
                <div className="col col-12 col-lg-8">
                  <div className="form-check">
                    ...
                    >
                      ...
                      <p>
                        {data.countryNameList} · Counsel: {data.counselName} ·{" "}
...
export default SuperAdminCaseLawYouListsComponent;
