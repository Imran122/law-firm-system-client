...
const SuperAdminLawsContributorsListsComponent = () => {
  const [lawsList, setLawsList] = useState([]);
  const { isLoading, setIsLoading } = useAuth();
  useEffect(() => {
    fetch(
      `${process.env.REACT_APP_API}/api/get-contribute-laws-by-contributor`,
      {
        method: "GET",
        headers: {
  ...
  const updatePaidOrFreeSatus = (id, status) => {
    ...
    fetch(`${process.env.REACT_APP_API}/api/update-status-paid-free`, {
      ...
      .then((data) => {
        if (data) {
          setIsLoading(false);
          navigate("/dashboard/all-laws/by-contributors", { replace: true });
        }
      });
  };
  ...
  const applyMultiFreePaidSatus = (status) => {
    ...
    fetch(`${process.env.REACT_APP_API}/api/update-multi-status`, {
      ...
      .then((data) => {
        if (data) {
          setIsLoading(false);
          navigate("/dashboard/all-laws/by-contributors", {
            replace: true,
          });
        }
      });
...
export default SuperAdminLawsContributorsListsComponent;
