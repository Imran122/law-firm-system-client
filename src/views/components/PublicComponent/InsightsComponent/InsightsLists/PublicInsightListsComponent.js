...
import InsightListData from "./InsightListData";

const PublicInsightListsComponent = () => {
  const { user } = useAuth();
  const [favorite, setFavorite] = useState(false);
  const toggleFavorite = () => setFavorite((prev) => !prev);
  const [insightList, setInsightList] = useState([]);
  useEffect(() => {
    fetch(`${process.env.REACT_APP_API}/api/get-insight-list`, {
      method: "GET",
      headers: {
  ...
  return (
    <>
      <div className="col col-12 col-lg-10 article-lists-bg">
        <div className="lists-title-number">
          <h2>
            Number of results: <span>38</span>
          </h2>
        </div>
        {insightList.map((data) => (
          <InsightListData item={data} key={data._id}></InsightListData>
        ))}
...
export default PublicInsightListsComponent;
